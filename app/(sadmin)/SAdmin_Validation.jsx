...
export default function SAdmin_Validation() {
  ...
  const filteredReports = useMemo(() => {
    ...
    return reports.filter((report) => {
      const matchesFilter =
        ...
        (selectedFilter === "User Reports" && report.submittedRole === "User");
  ...
  const adminCreatedCount = reports.filter(
    (item) => item.submittedRole === "Admin"
  ).length;
  ...
  const handleVerify = (report) => {
    updateReportStatus(
      report.id,
      "Verified",
      "Report verified by SuperAdmin after final review."
    );
  };
  ...
  const handleReject = (report) => {
    updateReportStatus(
      report.id,
      "Rejected",
      "Report rejected by SuperAdmin after validation review."
    );
  };
  ...
  const handleMapAndVerify = (report) => {
    updateReportStatus(
      report.id,
      "Verified",
      "Report mapped and verified by SuperAdmin."
    );
  };

  const handleAddSAdminReport = () => {
    const newReport = {
      id: `ARG-${Date.now().toString().slice(-4)}`,
      title: "SuperAdmin-created incident report",
      category: "Public Safety Incidents",
      type: "Official SuperAdmin Report",
      location: "Poblacion, Argao, Cebu",
      barangay: "Poblacion",
      ...
      status: "Verified",
      details:
        "This is an official SuperAdmin-created report. It is automatically verified and visible on the incident map.",
      photo: "sadmin_report.jpg",
      aiScore: 100,
      sentiment: "Official",
      credibilityReview: "SuperAdmin-created report. Automatically verified.",
      severity: "Medium",
      submittedAt: "Apr 29, 2026 • Current Time",
      verifiedBy: "SuperAdmin",
      verifiedAt: "Apr 29, 2026 • Current Time",
      remarks: "Official SuperAdmin report. Auto verified and mapped.",
      mapVisible: true,
    };

    setReports((prev) => [newReport, ...prev]);
    showMessage("SuperAdmin report added and automatically verified.");
  };
        ...
        >
          <View style={styles.headerCard}>
            <View style={styles.headerTextBox}>
              <Text style={styles.pageTitle}>Validation Management</Text>
              <Text style={styles.pageSubtitle}>
                Review submitted reports, verify valid incidents, reject invalid
                reports, and monitor admin-created reports.
              </Text>
            </View>

            <TouchableOpacity
              style={styles.addButton}
              onPress={handleAddSAdminReport}
              activeOpacity={0.85}
            >
              <Ionicons name="add" size={18} color="#FFFFFF" />
              <Text style={styles.addButtonText}>Add Official Report</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.filterCard}>
            <View style={styles.searchBox}>
              ...
              <TextInput
                ...
                placeholder="Search by ID, type, location, submitter, status..."
          ...
          <View style={styles.reportListCard}>
            <View style={styles.listHeader}>
              <View>
                <Text style={styles.sectionTitle}>Validation Queue</Text>
                <Text style={styles.sectionSubtitle}>
                  SuperAdmin can oversee all report validation records.
                </Text>
              </View>
